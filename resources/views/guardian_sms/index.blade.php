                    ...
                    <tbody>
                        @foreach($logs as $log)
                            <tr class="border-b hover:bg-gray-50">
                                <td class="px-4 py-3 font-medium">{{ $log->guardian_name }}</td>
                                <td class="px-4 py-3">{{ $log->guardian_phone }}</td>
                                <td class="px-4 py-3">{{ $log->message }}</td>
                                <td class="px-4 py-3">{{ $log->student->name ?? 'N/A' }}</td>
                                <td class="px-4 py-3">{{ \Carbon\Carbon::parse($log->sent_at)->format('M d, Y h:i A') }}</td>
                            </tr>
                        @endforeach
                    </tbody>
                </table>
